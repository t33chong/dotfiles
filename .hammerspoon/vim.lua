...
function alertN() return alert('^', '#FF0000') end
function alertV() return alert('v', '#FFFF00') end
...
-- I - insert at beginning of line {{{3
...
function visual:entered() alertV() end
-- }}}2

-- <cmd-esc> - exit Visual mode {{{2
visual:bind({'cmd'}, 'escape', function()
    visual:exit()
    normal:enter()
    alertN()
    hs.eventtap.keyStroke({}, 'Right')
...
local disabledApps = {'iTerm2', 'Terminal'}
for n = 1, #disabledApps do
  hs.window.filter.new(disabledApps[n])-- {{{1
